...
createResponder({
    ...
    async run(interaction) {
        ...
        try {
            ...
            if (err instanceof DiscordAPIError && err.message.includes("parent_id[CHANNEL_PARENT_INVALID]: Category does not exist")) {
                await interaction.reply({ content: `${emojis.error} | Ops! Parece que você excluiu a categoria que foi configurada no meu banco de dados.\n-# Defina uma nova categoria para os canais de ticket!`, ephemeral: true });
            }
        }
    }
});